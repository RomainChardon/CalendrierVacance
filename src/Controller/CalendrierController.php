...
class CalendrierController extends AbstractController
{
    ...
    private function createListDays(DatePeriod $periode, GroupeRepository $repoGroupe): array
    {
        foreach ($periode as $date) {
            ...
            $siSamedi = false;
            $siDimanche = false;
            if ('Samedi' == $this->traductionJour($jourAfficher)) {
                $siSamedi = true;
            } elseif ('Dimanche' == $this->traductionJour($jourAfficher)) {
                $siDimanche = true;
            }
            ...
            foreach ($this->createListeFerier($this->anneeUtiliser) as $jourFerier) {
                ...
                foreach ($listeGroupe as $groupe) {
                    ...
                    foreach ($groupeUser as $user) {
                        ...
                        $listDetailUser[$nomUser.'-'.$prenomUser] = [
                            'nomUser' => $nomUser,
    ...
    private function createListDays2(DatePeriod $periode, GroupeRepository $repoGroupe): array
    {
        foreach ($periode as $date) {
            ...
            $siSamedi = false;
            $siDimanche = false;
            if ('Samedi' == $this->traductionJour($jourAfficher2)) {
                $siSamedi = true;
            } elseif ('Dimanche' == $this->traductionJour($jourAfficher2)) {
                $siDimanche = true;
            }
